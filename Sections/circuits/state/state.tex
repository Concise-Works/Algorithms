...
\begin{Def}[Memory Address] 

  A \textbf{Memory Address} is a unique identifier for a specific location in a memory device, often represented
  by binary numbers, hexadecimal numbers, or alphanumeric strings.
\end{Def}

\noindent
Though this arrangement is more compact, we reverted back to only being able to store one bit per location.
To fix this, we can combine the ideas from registers and grids: