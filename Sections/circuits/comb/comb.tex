...
\begin{Def}[Karnaugh Maps]
    ...
    \noindent
    1) Shows the truth table for inputs $A$, $B$, and $C$ with output $F$. 2) Shows the corresponding K-map. \textbf{Note:}
    with this implementation, the columns and rows are cyclical, meaning the leftmost and rightmost columns are adjacent, as are the top and bottom rows.
    Hence with 3), we can represent a 3 input truth table as a 3D cube. The red arrows in both 2) and 3) indicate adjacency. We can extend this to 4 inputs:\\
