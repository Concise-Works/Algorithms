...
\noindent
\subsection{Branching Logic: Multiplexers \& Decoders}
...
\noindent 
To be clear, Logic gates and MUXes may both be used to implement boolean functions,
it's a matter of which use-case is more appropriate:

\begin{theo}[MUX vs. Logic Gates]

    Logic gates help with basic/dedicated functions, such as needing a AND, OR, NOT 
    gate, or a simple routine (algorithm). MUXes help with selecting between multiple data inputs, i.e., 
    large if-else/branching logical statements.

    Depending on the function implementation's $t_{PD}$ and size requirements, one may be more appropriate than the other
    based on the schematics at hand.
\end{theo}

\begin{Note} \textbf{Note:} In real world applications, MUXes carry 
  significant overhead compared to logic gates. They are complex devices for simple selection use 
  cases, meaning they are not as flexible as logic gates. This is an important consideration when it comes to the design costs/efficiency of a circuit.
\end{Note}

\noindent
We discussed a device (MUX) that selects the row of a truth table where a specific piece of data lives. This is good
for one output; But to allow for $n$ outputs, we'll add another set of $n$ lines (columns) to each row. Each column 
leads to an output:

\begin{figure}[ht!]
  \centering
  \includegraphics[width=1\textwidth]{Sections/circuits/comb/dec.png}
  \caption{1) Shows an altered 4-to-1 MUX from Definition (\ref{def:mux}) with the addition of 2 columns (outputs) $A$ and $B$ per row.
  2) Shows $S_0=0$ and $S_1=1$, thus selecting row 2, outputting $A=1$ and $B=1$.}
  \label{fig:dec}
\end{figure}
...
\begin{Def}[Decoder]
  ...
  \label{def:dec}
    ...
    A \textbf{decoder} is a combinational logic device that takes $n$ control inputs and decodes them
    to $2^n$ possible unique outputs (explained on the next page). E.g., Figure (\ref{fig:dec}) shows a 2-to-4 decoder, 2 inputs leading to 4 outputs.
    
    This differs from the MUX naming convention where $n$ select inputs lead to 1 output ($2^n$-to-1 MUX) vs. ($n$-to-$2^n$ Decoder).
\end{Def}

\noindent
In Figure (\ref{fig:dec}), we see that we'll always get 1 from each input,
which isn't very interesting; 

\begin{theo}[Decoder PDN-line Manipulation]

  To achieve $2^n$ output on a decoder, PDNs are attached to output-lines driving high (1) lines to low (0).
  Each row mix-and-matches PDNs to each intersection just how a truth table would with 0s and 1s for 
  unique combinations.\\
  \textbf{Note:} PDNs only effect their intersection, not the entire column.
\end{theo}

\begin{figure}[ht!]
  \centering
  \includegraphics[width=1\textwidth]{Sections/circuits/comb/dec2.png}
  \caption{1) Shows the 2-to-4 decoder from Figure (\ref{fig:dec}) now with PDNs: $[0;0]$ has 0 PDNs,
  $[0;1]$ and $[1;0]$ have 1 PDN each, and $[1;1]$ has 2 PDNs. 2) Shows $S_0=0$ and $S_1=1$, thus selecting row 2, outputting $A=1$ and $B=0$ because of it's PDN.
  \textbf{Notice:} That $A$'s line isn't effected by the other PDNs below, thus it's output remains high.}
  \label{fig:dec-pdn}
\end{figure}

\noindent
\subsection{Creating the ALU: Addition \& Subtraction Circuits}
We now know enough to create a combinational logic circuit that performs addition. Recall the information provided in
Theorem (\ref{theo:adding_binary}) about binary addition. We now build intuition:

\begin{theo}[32-bit Full Adder Strategy (Logic Gates)]

    We approach the problem truth-table first. To add two binary numbers $A$ and $B$, we need at least 2 inputs;
    However, we need to account for carry-in bits $C_{in}$ from previous additions\\
    (3 inputs). We also need to pick between sum bit $S$ and carry-out bit $C_{out}$ (2 outputs). 
    
    Logically, we only have a carry if at least 2 of the 3 inputs are 1;
    Hence,\\ 
    $C_{out} = (A\land B) \lor (A\land C_{in}) \lor (B\land C_{in})$. For the sum bit, at most 1 input can be 1; Hence,
    $S = A \oplus B \oplus C_{in}$ (XOR). 
    
    This only adds 1 bit; To extend to 32-bits, we chain 32 of these full adders (FA) together:
    Break $A$ and $B$ into 32 individual bits, $A_{0}$ to $A_{31}$ and $B_{0}$ to $B_{31}$. The carry-out of each FA becomes the carry-in of the next FA.
    We may use the last carry-out $C_{out}$ as an overflow bit.
\end{theo}
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=.9\textwidth]{Sections/circuits/comb/fa.png}
  \caption{Full Adder (FA) truth table, $A$, $B$, and $C_{in}$, with outputs $C_{out}$ and $S$.}
  \label{fig:fa}
\end{figure}
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=1\textwidth]{Sections/circuits/comb/fa2.png}
  \caption{1) $S$ logic gates, 2) $C_{out}$ logic gates, 3) combining $S$ and $C_{out}$ logic gates to form a full adder. 4) diagram 
  representation of FA, 5) chaining 32 FAs to create a 32-bit adder. \cite{edgington2017adder} }
  \label{fig:fa-chain}
\end{figure}
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=1\textwidth]{Sections/circuits/comb/fa3.png}
  \caption{1) Shows our desired logic (truth-table) 2) Shows a 3-to-8 Decoder, with select lines 
  $A$, $B$, and $C_{in}$, driving 8 outputs, leading to outputs $S$ and $C_{out}$ via OR gates. \textbf{Note:} The inverters just before the outputs; This
flips our PDNs low outputs to high for our desired logic.}
  \label{fig:fa-mux-chain}
\end{figure}
...
\begin{theo}[32-bit Subtraction Strategy via 2's Complement]

    Recall, $3-2=3+(-2)$; Thus, altering our 32-bit adder and negating, say $B$, allows 
    us to perform subtraction. We negate using Definition (\ref{def:signed_binary_numbers_twos_complement}), two's complement:
    Invert every bit in the number, then add 1. I.e., 
    ...
    \noindent
    Instead of building a separate subtraction circuit, we use our initial $C_{in}$ as a 
    control line choosing between addition (0) and subtraction (1); Connect the control line
    to all outgoing $B$ lines replacing the ORs with XOR gates to invert $B$ when subtracting.
\end{theo}
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=1\textwidth]{Sections/circuits/comb/sub.png}
  \caption{ Here shows a similar FA string from Figure (\ref{fig:fa-chain}), with the addition of XOR gates
  after each $B$ input supplemented by the initial $C_{in}$ line (control line). \cite{villanova_alu}}
  \label{fig:fa-subtract}
\end{figure}

\begin{theo}[$n$-bit AND and OR Operations]

  \label{theo:n-bit-and-or}

    To create $n$-bit AND and OR operations, we chain $n$ individual AND and OR gates respectively
    to each bit from $A$ and $B$. E.g., for AND, gate 0 takes in $A_0$ and $B_0$ and so on.
\end{theo}

\begin{figure}[ht!]
  \centering
  \includegraphics[width=1\textwidth]{Sections/circuits/comb/and_or_bit.png}
  \caption{ (1) and (3) both demonstrate the AND and OR chain discussed in 
  Theorem (\ref{theo:n-bit-and-or}) respectively. 2) shows an abstraction or \textbf{`interface'}
  of the AND circuit; The slash (``/") followed by the $32$ indicates that the line is actually 32 individual lines (bits). \cite{villanova_alu} }
  \label{fig:32-bit_and_or}
\end{figure}
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=1\textwidth]{Sections/circuits/comb/alu.png}
  \caption{ Here shows a basic ALU that can perform addition, subtraction, AND, and OR operations.
  Starting from the left, we have 2 inputs $A$ and $B$ both of variable bit-length $n$.
  At the top we have our 2-bit OPCODE. The OPCODE selects which operation to perform via a 2-4 decoder (DEC),
  leading to 2 control lines (Control$_0$ and Control$_1$). Control$_0$ ($C_0$), if $C_0=0$ then (AND and ADD) are selected, else (OR and SUB) are selected
  Control$_1$ ($C_1$), if $C_1=0$ then the boolean operations (AND or OR) is the Result; Otherwise (ADD or SUB) is the Result. The ADD and SUB operations share the same 32-bit adder/subtractor circuit from Figure (\ref{fig:fa-subtract})
  E.g., OPCODE = 10, $C_0=1$ and $C_1=0$, performs subtraction. We have additional outputs, \textbf{`Zero'}, which determines if the output is zero; 1 (true) and 0 (false).
  Then \textbf{`Sign'}, which relies on two's complement formatting to acquire the integer sign $n_0$ the left-most bit. And then of course
  our `Carry' and `Overflow' from our modified FA. \cite{villanova_alu}
  }
  \label{fig:alu}
\end{figure}