...
\noindent
Say $u\to v$ and $x\to y$ are at the exact same distance from $s$ at the point of exiting $S$ subset of paths, i.e., say $s\to v\cong s\to y$. Then to go from $y\to v$ must take some additional step.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{./Sections/dstra/w_dist.png} % First image
    \caption{A weighted graph and its shortest paths (highlighted in green).}
