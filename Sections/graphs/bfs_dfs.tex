...
\begin{figure}[h]
    ...
    \end{center}
     \caption{A BFS tree traversal preformed on a graph with each level enumerated in red}\label{fig:bfs}
  \end{figure}
...
\begin{figure}[h]
    \begin{center}
      \includegraphics[height=2.3in]{./Sections/graphs/bfs_q_ana.png}
    \end{center}
     \caption{Time \& Space complexity $O(m+n)$, for $m$ edges and $n$ nodes.}\label{fig:bfs_q_ana}
  \end{figure}
  \begin{Proof}[Claim 1 for BFS]
    Let $s$ be the root of the BFS tree, then:
    \textbf{Proof:} Induction on the distance from $s$ to $u$.\\
    \textbf{Base case} ($u = s$): The code sets Level[$s$] = 0, and there is no path to find since the path has length 0. \textbf{Induction hypothesis:} For every node $u$ at distance $\leq i$, Claim 1 holds. \textbf{Induction step:}
    \begin{itemize}
