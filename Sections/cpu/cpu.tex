...
\noindent
This section is quite definition-rich to get us started, but we quickly shift straight 
into discussing our first data-structures, the \textbf{Stack \& Heap}, and associated algorithms.

\begin{Def}[Computer]

    A \textbf{computer}, is a device which takes input and computes an output 
    based on a set of instructions called a \textbf{program}.
\end{Def}

\begin{Def}[Central Processing Unit (CPU)]

    The \textbf{CPU (Central Processing Unit)}, is a hardware component that \emph{computes} instructions within a \emph{computer}.
    ...
    \noindent
    ISA's define an instruction set, which allows for flexibility in hardware performance needs.
    Popular CPU architectures include \textbf{x86\_64, and ARM64 (64-bit)} systems, originating from \textbf{x86 and ARM (32-bit)} systems (we'll discuss what a `\emph{bit}' is later on). 
    
    The implementation of a CPU on a circuit board (a medium to house the CPU) is called a \textbf{microprocessor}. Multiple CPUs on a single circuit board are \textbf{multi-core processors}, where each \emph{core} (CPU)
    makes up one processing unit.
\end{Def}

\begin{Tip}
The CPU is often compared to as the \textbf{brain} of the computer, \textbf{but} really the CPU is the computer itself, as it does the \emph{computing}.
Other things we associate with computers (e.g., monitor, keyboard, mouse, speakers, etc.) are just peripherals that interact with the CPU.
\end{Tip}
...
\begin{Def}[CPU Anatomy: Von Neumann Architecture]
    ...
    \begin{itemize}
        ...
        \item \textbf{Memory Unit (MU):} Manages data storage and retrieval.
    \end{itemize}
    \noindent
...
\begin{Def}[Registers \& RAM]

    \label{def:registers}

    \textbf{Registers} are small, high-speed storage locations within the CPU that hold data temporarily during execution.
    The two main methods of use are:
    \begin{itemize}
        \item \textbf{General-Purpose Registers (GPR):} Hold general data storage and manipulation. They may 
        be accompanied by other specialized registers.
        \item \textbf{Random Access Memory (RAM):} Is when multiple registers are combined to 
        form a larger units of memory. Though this is slower as we have to find where data is stored.
    \end{itemize}
    \noindent
\end{Def}

\begin{Tip}
It's akin to having pockets (registers) to quickly access items, versus a backpack (RAM) for larger items that take longer to swap out and use.
\end{Tip}

\newpage

\noindent
The following is an example of the primary registers in the x86-32 (IA-32) architecture.
\begin{table}[h]
\centering
\begin{tabular}{|l|c|l|}
\hline
...
\hline
\end{tabular}
\caption{Primary registers of the x86-32 (IA-32) architecture. \textbf{Note:} Registers are prefixed with \underline{`E' for 32-bit, `R' for 64-bit in x86-64} (The R isn't shown as this is a 32-bit architecture).}
\end{table}
...
\begin{Def}[Machine Code \& Compilation]
    ...
    \begin{itemize}
        ...
        \item \textbf{Data Segment:} The part of the program which contains variables storing data.
        \item \textbf{Machine Code:} The compiled (translated) code of the program (data \& text), which is executed by the CPU.
    \end{itemize}
...
\begin{Example}[Student Class]
    ...
    Consider a simple `Student' class (blueprint) in an object-oriented programming language:
